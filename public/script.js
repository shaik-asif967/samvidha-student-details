...
document.getElementById("docForm").addEventListener("submit", e => {
  ...
  let url = type === "PHOTO"
    ? `https://iare-data.s3.ap-south-1.amazonaws.com/uploads/STUDENTS/${roll}/${roll}.jpg`
    : `https://iare-data.s3.ap-south-1.amazonaws.com/uploads/FIELDPROJECT/2024-25_${roll}_FM.pdf`;
  ...
  if (url.endsWith(".pdf")) {
    ...
    box.innerHTML = `<img src="${url}" alt="Document" style="max-width:100%;">`;
  }
});
  ...
  try {
    ...
    data.results.forEach(exam => {
      ...
      exam.subjects.forEach(sub => {
        html += `<tr><td>${sub.subject}</td><td>${sub.credits}</td><td>${sub.grade}</td><td>${sub.points}</td></tr>`;
      });
      html += `</table>`;
      html += `<p>SGPA: ${exam.sgpa || "N/A"} | CGPA: ${exam.cgpa || "N/A"}</p><hr>`;
    });
    box.innerHTML = html;
